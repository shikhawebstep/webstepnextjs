    ...
    <section className="relative w-full min-h-screen flex flex-col overflow-hidden bg-white text-slate-900 font-roboto">
      ...
      <div className="relative z-10 w-full overflow-hidden border-b border-slate-100 bg-white/50 backdrop-blur-sm py-3">
        ...
        >
          {doubled.map((item, i) => (
            <span
              key={i}
              className="font-mono text-[9px] tracking-[0.3em] uppercase text-slate-400 px-12 whitespace-nowrap flex items-center gap-3"
            >
              {item}
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center text-center px-6 md:px-12 py-20 max-w-7xl mx-auto">
        ...
        >
          ...
          <span className="text-[9px] font-black uppercase tracking-[0.4em]">Defining The Next Standard</span>
        </motion.div>
        ...
        <div className="relative mb-10">
            ...
            className="font-heading text-6xl md:text-8xl lg:text-9xl font-black text-slate-900 leading-[0.9] tracking-tighter uppercase"
            >
            Crafting <br />
            ...
            <span className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[15vw] font-black text-slate-50 opacity-[0.02] pointer-events-none select-none uppercase -z-10 tracking-tighter">
                Innovation
            </span>
        </div>
        ...
        >
          <button className="group relative px-12 py-5 bg-slate-900 text-white rounded-2xl text-[11px] font-black uppercase tracking-[0.3em] overflow-hidden shadow-2xl shadow-slate-900/20 hover:-translate-y-1 transition-all duration-500">
          ...
          <button className="px-12 py-5 bg-white border border-slate-200 text-slate-900 rounded-2xl text-[11px] font-black uppercase tracking-[0.3em] hover:border-pink-500 hover:text-pink-600 transition-all duration-300">
            View Case Studies
          </button>
        </motion.div>
      </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-white/40 backdrop-blur-3xl rounded-[3rem] border border-white/50 shadow-2xl">
          {[
            ...
            >
              ...
              <span className="block font-mono text-[8px] tracking-[0.3em] uppercase text-slate-400">
                {stat.label}
              </span>
