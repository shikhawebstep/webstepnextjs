...
const MobileElevate = () => {
  return (
    <section className="py-16 md:py-24 bg-white relative z-10 px-6 overflow-hidden font-roboto">
      ...
      {/* Background patterns */}
      <div className="absolute inset-0 opacity-[0.03] pointer-events-none" style={{ backgroundImage: 'radial-gradient(#3b82f6 0.5px, transparent 0.5px)', backgroundSize: '32px 32px' }}></div>

      <div className="container mx-auto relative z-10">
        <div className="bg-blue-50 rounded-[3rem] p-12 md:p-20 relative overflow-hidden">
          {/* Subtle glow */}
              ...
              <AnimatedSection delay={0.1} direction="right">
                <div className="mb-10">
                  <h2 className="text-3xl md:text-5xl font-black text-slate-900 mb-6 capitalize tracking-tight leading-tight">
                    Hire Certified <span className="text-blue-600">Mobile Developers</span>
                  </h2>
                  <p className="text-slate-600 text-lg leading-relaxed mb-8">
                    Our mobile app developers are experts in building high-performing applications that engage users and drive results. We focus on modern design and technical excellence.
                  </p>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                  {[
                    { icon: <FaMobileAlt className="text-blue-500" />, label: "Native Apps" },
                    { icon: <FaLayerGroup className="text-indigo-500" />, label: "Hybrid Stack" },
                    { icon: <FaCheckCircle className="text-blue-500" />, label: "Store Ready" }
                  ].map((item, index) => (
                ...
                <button className="mt-12 px-10 py-5 bg-slate-900 text-white rounded-2xl text-[11px] font-black capitalize tracking-[0.25em] shadow-2xl hover:bg-blue-600 transition-all duration-300">
                  Launch Your App
                </button>
              </AnimatedSection>
            </div>
